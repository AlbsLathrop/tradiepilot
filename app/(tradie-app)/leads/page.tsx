...
export default function LeadsPage() {
  ...
  useEffect(() => {
    ...
    const fetchLeads = async () => {
      try {
        ...
        // const res = await fetch(`/api/leads?tradieConfigId=${session.user.tradieConfigId}`)
        // const data = await res.json()
        // Temporarily use sample data
        setLeads(Array.isArray(sampleLeads) ? sampleLeads : [])
      } catch (err) {
  ...
  if (!leads || !Array.isArray(leads)) {
    return (
      <div className="p-4">
        <div className="bg-red-500/20 border border-red-500/50 rounded-xl p-4 text-red-400">
          <p className="font-semibold">Invalid leads data</p>
        </div>
      </div>
    )
  }
      ...
      <div className="space-y-3">
        {!filteredLeads || !Array.isArray(filteredLeads) ? (
          <div className="bg-[#1F2937] rounded-xl p-6 border border-[#374151] text-center">
            <p className="text-[#9CA3AF]">No leads available</p>
          </div>
          ...
          filteredLeads.map(lead => {
            ...
            const dateStr = lead?.dateReceived || new Date().toISOString()
            return (
              <div
                key={lead.id}
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <div>
                    <h3 className="text-base font-bold text-[#F9FAFB]">{lead?.name || 'Unnamed Lead'}</h3>
                    <p className="text-sm text-[#9CA3AF] mt-1">
                      {lead?.service || 'Service TBD'} • {lead?.suburb || 'Location TBD'}
                    </p>
                  </div>
                  <span
                ...
                <div className="text-xs text-[#6B7280] space-y-0.5">
                  <p>Received: {new Date(dateStr).toLocaleDateString()}</p>
