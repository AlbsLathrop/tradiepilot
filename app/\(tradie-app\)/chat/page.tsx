...
export default function ChatPage() {
  ...
  const sendMessage = async (text: string) => {
    if ((!text.trim() && !pendingAttachment) || loading || uploading) return;

    const userMsg: Message = {
      id: Date.now().toString(),
      role: 'joey',
      content: text || (pendingAttachment ? `📷 ${pendingAttachment.file.name}` : ''),
      timestamp: new Date(),
    };
    ...
    try {
      ...
      const res = await fetch('/api/alfred', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: text || `Uploaded a ${mediaType?.toLowerCase()}: photo`,
          mediaUrl,
          mediaType,
          tradieConfigId,
      ...
      // Update message with media if it was uploaded
      if (mediaUrl) {
        setMessages(prev => prev.map(m =>
          m.id === userMsg.id
            ? { ...m, mediaUrl, mediaType }
            : m
        ));
      }
  ...
  return (
    <div className="flex flex-col h-screen bg-[#111827]">
      ...
      {/* Attachment preview */}
      {pendingAttachment && (
        <div className="px-4 py-3 bg-[#1a1a2e] border-t border-[#374151] flex items-center gap-3">
          <div className="relative">
            <img
              src={pendingAttachment.previewUrl}
              alt="Preview"
              className="w-16 h-16 object-cover rounded-lg border-2 border-[#F97316]"
            />
            {uploading && (
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-[#9CA3AF] truncate">{pendingAttachment.file.name}</p>
          ...
          <button
            ...
            disabled={uploading}
            className="text-[#9CA3AF] hover:text-red-500 disabled:opacity-40 transition-colors p-1"
          >
            <X size={18} />
          </button>
        </div>
      ...
      <div className="px-4 py-4 bg-[#0F0F0F] border-t border-[#374151]">
        <div className="flex items-center gap-2 bg-[#1F2937] border border-[#374151] rounded-2xl px-3 py-2">
          <input
            ...
            disabled={uploading || pendingAttachment !== null}
          />
          <button
          ...
          <input
            ...
            placeholder={pendingAttachment ? 'Add a message (optional)...' : 'Message ALFRED...'}
